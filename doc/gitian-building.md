...
*Setup instructions for a gitian build of Helium using a Debian VM or physical system.*
...
to https://github.com/heliumchain/helium.
...
- [Building Helium](#building-helium)
...
Clone the git repositories for helium and gitian and then checkout the helium version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the helium repository
...
To build Helium (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the helium repository.
...
```bash
    ...
    From https://github.com/helium-crypto/helium
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/helium-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/helium-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/helium-build.assert
```
...
[helium/gitian.sigs](https://github.com/heliumchain/helium/gitian.sigs/) repository, or if that's not possible to create a pull
request.
