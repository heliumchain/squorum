Helium Core version 3.0.6 is now available from:
...
zHLM Spending Fix
---------------------
zHLM that were minted between block 891730 and 895400 were experiencing an error initializing the accumulator witness data correctly, causing an inability to spend those mints. This has been fixed.
...
- #408 `beae959` Fix "accumulator does not verify" when spending zHLM. (presstab)
...
### Wallet
- #412 `2fb5f17` Double check tx size when creating zHLM tx's. (presstab)
