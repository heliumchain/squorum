...
bool IsBudgetCollateralValid(uint256 nTxCollateralHash, uint256 nExpectedHash, std::string& strError, int64_t& nTime, int& nConf, bool fBudgetFinalization)
{
    ...
    for (const CTxOut &o : txCollateral.vout) {
        ...
        if (fBudgetFinalization) {
            ...
            // Note: there are still old valid budgets out there, but the check for the new 5 SQR finalization collateral
            //       will also cover the old 50 SQR finalization collateral.
...
bool CBudgetVote::SignatureValid(bool fSignatureCheck)
{
    ...
    if (pmn == NULL) {
        if (fDebug){
            LogPrint("debug","CBudgetVote::SignatureValid() - Unknown Masternode - %s\n", vin.prevout.hash.ToString());
        }
        return false;
    }
...
bool CFinalizedBudgetVote::SignatureValid(bool fSignatureCheck)
{
    ...
    if (pmn == NULL) {
        LogPrint("debug","CFinalizedBudgetVote::SignatureValid() - Unknown Masternode %s\n", vin.prevout.hash.ToString());
        return false;
    }
