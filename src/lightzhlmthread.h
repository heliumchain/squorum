...
private:
                ...
                if (!pIndex) {
                    ...
                    if (blockHeight >= Params().Zerocoin_Block_V2_Start()) {
                        ...
                        if (!res) {
                            ...
                            ss << heightStop;
                            if (genWit.getPfrom()) {
                                LogPrintf("%s pushing message to %s ", "helium-light-thread", genWit.getPfrom()->addrName);
                                genWit.getPfrom()->PushMessage("pubcoins", ss);
                            } else
                                LogPrintf("%s NOT pushing message to %s ", "helium-light-thread", genWit.getPfrom()->addrName);
                        }
                    } else {
                        // Rejects only the failed height
                        rejectWork(genWit, blockHeight, NON_DETERMINED);
    ...
    void rejectWork(CGenWit& wit, int blockHeight, uint32_t errorNumber) {
        if (wit.getStartingHeight() == blockHeight){
            LogPrintf("%s rejecting work %s , error code: %s", "helium-light-thread", wit.toString(), errorNumber);
            CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
