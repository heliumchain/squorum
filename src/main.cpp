...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    ...
    if (CZHlmStake* zHLM = dynamic_cast<CZHlmStake*>(stake)) {
        ...
        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zHLM stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZHLM() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
    ...
    if (isPoS) {
        ...
        if (isBlockFromFork) {
            ...
            if(hasZHLMInputs){
                LogPrintf("It has ZHLM Inputs!!\n");
                for (CTxIn zHlmInput : zHLMInputs) {
                    CoinSpend spend = TxInToZerocoinSpend(zHlmInput);
