                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zHLM ought to be enough for anybody." - Bill Gates, 2017
...
void PrivacyDialog::sendzHLM()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Helium Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zHelium was spent successfully update the addressbook with the label
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zHelium Spend #: ") + QString::number(nCount) + ", ";
