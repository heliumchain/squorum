                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    ui->zHLMpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzHLMSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zHLMpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzHLM_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zHLM, true));
        if (!ctx.isValid()) {
...
void PrivacyDialog::on_pushButtonSpendzHLM_clicked()
{
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zHLM, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zHLM
        sendzHLM();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zHLM
    sendzHLM();
}
...
void PrivacyDialog::sendzHLM()
{
    ...
    double dAmount = ui->zHLMpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zHLM is requested
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZHLM_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zHLM require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zHLM"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zHLM was spent successfully update the addressbook with the label
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zHLM Spend #: ") + QString::number(nCount) + ", ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zHLM "));
    ui->labelzHLMAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zHLM </b> "));
...
{
    ui->labelzHLMSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzHLM->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zHLM
        ui->pushButtonMintzHLM->setEnabled(false);
        ui->pushButtonMintzHLM->setToolTip(tr("zHLM is currently disabled due to maintenance."));

        // Spend zHLM
        ui->pushButtonSpendzHLM->setEnabled(false);
        ui->pushButtonSpendzHLM->setToolTip(tr("zHLM is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zHLM
        ui->pushButtonMintzHLM->setEnabled(true);
        ui->pushButtonMintzHLM->setToolTip(tr("PrivacyDialog", "Enter an amount of HLM to convert to zHLM", 0));

        // Spend zHLM
        ui->pushButtonSpendzHLM->setEnabled(true);
        ui->pushButtonSpendzHLM->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
