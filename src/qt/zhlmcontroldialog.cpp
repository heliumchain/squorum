...
std::list<std::string> ZHeliumControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZHeliumControlDialog::listMints;

ZHeliumControlDialog::ZHeliumControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZHeliumControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZHeliumControlDialog::~ZHeliumControlDialog()
{
    delete ui;
}

void ZHeliumControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZHeliumControlDialog::updateList()
{
...
void ZHeliumControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZHeliumControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZHelium_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZHeliumControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZHeliumControlDialog::GetSelectedMints()
{
...
void ZHeliumControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
