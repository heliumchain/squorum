...
// Copyright (c) 2015-2017 The XHLM developers
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sHLMPercentage, QString& szHLMPercentage)
{
    int nPrecision = 2;
    ...
    szHLMPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sHLMPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->labelzHLMPercent->setText(szPercentage);
    ...
    bool showHLMAvailable = settingShowAllBalances || pivAvailableBalance != nTotalBalance;
    bool showWatchOnlyHLMAvailable = watchOnlyBalance != nTotalWatchBalance;
    bool showHLMPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyHLMPending = watchUnconfBalance != 0;
    bool showHLMLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyHLMLocked = nWatchOnlyLockedBalance != 0;
    ...
    ui->labelWatchAvailable->setVisible(showHLMAvailable && showWatchOnly);
    ...
    ui->labelWatchPending->setVisible(showHLMPending && showWatchOnly);
    ...
    ui->labelWatchLocked->setVisible(showHLMLocked && showWatchOnly);
    ...
    bool showzHLMAvailable = settingShowAllBalances || zerocoinBalance != matureZerocoinBalance;
    bool showzHLMUnconfirmed = settingShowAllBalances || unconfirmedZerocoinBalance != 0;
    bool showzHLMImmature = settingShowAllBalances || immatureZerocoinBalance != 0;
    ui->labelzBalanceMature->setVisible(showzHLMAvailable);
    ui->labelzBalanceMatureText->setVisible(showzHLMAvailable);
    ui->labelzBalanceUnconfirmed->setVisible(showzHLMUnconfirmed);
    ui->labelzBalanceUnconfirmedText->setVisible(showzHLMUnconfirmed);
    ui->labelzBalanceImmature->setVisible(showzHLMImmature);
    ui->labelzBalanceImmatureText->setVisible(showzHLMImmature);
    bool showPercentages = ! (zerocoinBalance == 0 && nTotalBalance == 0);
    ui->labelHLMPercent->setVisible(showPercentages);
    ui->labelzHLMPercent->setVisible(showPercentages);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("HLM")
    updateDisplayUnit();
}
