...
bool ZHLMModule::createInput(CTxIn &in, CZerocoinMint mint, uint256 hashTxOut){
    uint8_t nVersion = mint.GetVersion();
    if (nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
        // No v1 serials accepted anymore.
        return error("%s: failed to set zHLM privkey mint version=%d\n", __func__, nVersion);
    }
    CKey key;
    ...
    if (!key.Sign(hashTxOut, vchSig))
        throw std::runtime_error("ZHLMModule failed to sign hashTxOut\n");
...
PublicCoinSpend ZHLMModule::parseCoinSpend(const CTxIn &in, const CTransaction& tx){
...
bool ZHLMModule::validateInput(const CTxIn &in, const CTxOut &prevOut, const CTransaction& tx, PublicCoinSpend& ret){
