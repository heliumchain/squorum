...
class RESTTest (BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_greater_than(int(response_hex.getheader('content-length')), 192)
        response_hex_str = response_hex.read()
        assert_equal(encode(response_str, "hex_codec")[0:192], response_hex_str[0:192])

        # compare with hex block header
        response_header_hex = http_get_call(url.hostname, url.port, '/rest/headers/1/'+bb_hash+self.FORMAT_SEPARATOR+"hex", True)
        assert_equal(response_header_hex.status, 200)
        assert_greater_than(int(response_header_hex.getheader('content-length')), 192)
        response_header_hex_str = response_header_hex.read()
        assert_equal(response_hex_str[0:192], response_header_hex_str[0:192])
        assert_equal(encode(response_header_str, "hex_codec")[0:192], response_header_hex_str[0:192])
